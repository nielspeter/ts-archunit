...
**Related:** [018](./018-adoptable-discovery-surface.md) covers a different defect — findings that exist but are emitted where an agent cannot receive them. This one covers findings that are never produced at all. Split deliberately: 018 needs a ratchet first, this needs none. **This proposal now also owns the regression guard.** Review of [plan 0068](../plans/0068-close-vacuity-holes-in-dogfooding.md) moved it here: fixing four sites and guarding them in one commit removes the ordering problem and avoids a plan depending on an unreviewed proposal. The measured rule is structural, not name-based — `functions(p).that().resideInFolder(...).and().haveReturnType(/ArchViolation\[\]/).should().notContain(call(/^console\./))`, giving 24 subjects and 7 hits (all four defects plus three in `applyFilters`, which this proposal rules out of scope explicitly rather than by a selector that cannot see them).
