...
**Status:** **Absorbed by [plan 0070](../plans/completed/0070-a-rule-must-assert-something.md).** Its 0.22.0 work replaced the four `console.warn + return []` sites with one per-state `assertionAdvice()` read by `doctor`/`diagnose()`; its 0.23.0 makes the state fail. The one ask 0070 did **not** take is the `overrides`-key warning in `src/presets/shared.ts` — see 0070's Out of scope.
