...
**Version:** 0.26.0 · **Tests:** 2478 across 176 files · **Updated:** 2026-07-29
...
| ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------- |
| [0071](./0071-one-definition-of-a-module-edge.md) — Forward dependency conditions see every module edge | **DRAFT 2** — five-persona review against a working prototype; nine of draft 1's claims measured wrong, bug 0015 descoped to a diagnostic | ~2 days  | nothing — closes bug 0022 (0.27.0); 0015's diagnostic follows                                                     |
...
| [0015](../bugs/0015-allowlist-conditions-pass-vacuously-on-edgeless-subjects.md) — `only*` conditions pass on ∅                                       | **Open — DESCOPED by [plan 0071](./0071-one-definition-of-a-module-edge.md) draft 2.** The failing tier is withdrawn: measured, it reds correct code three ways — 12 of 13 rules `strictBoundaries` generates for one dependency-free shared file, a pure-entity innermost layer, and (worst) a layer whose only edge is `import type` under the `ignoreTypeImports` the docs recommend. Same precedent as the slice discovery guards withdrawn from 0.18.1. What ships later is a diagnostic aimed at the case that is actually silent — a **denylist** glob matching nothing; draft 1 aimed it at an allowlist typo that produces 96 violations. |
