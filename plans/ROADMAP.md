...
| ~~[0077](./completed/0077-doctor-promote-it.md) — `doctor`: promote it                            | **DONE in the next release, and reversed during its own investigation.** Draft 1 said retire, on the ground that `doctor` cannot load the shape the docs lead with. Measured: it cannot load a vitest-importing file (true), but `getting-started.md:10` says **the CLI is the default path** and `init` scaffolds a rule file with zero vitest imports — so `doctor` works on the documented default. It also reports **load failures, which `diagnose()` structurally cannot**, and a rule file that fails to load is zero coverage reported as success. Recommends promoting it to `--help` and replacing 'future undecided' with its scope. Still unblocks [0074](./0074-r3b-the-selector-glob-flip.md).                              | ~2 hours                   | a decision                              |
