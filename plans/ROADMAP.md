...
**Version:** 0.33.0 · **Tests:** 2726 across 196 files · **Updated:** 2026-07-31
**Spec:** `../ts-archunit-spec.md` · **Direction:** `ai-era-product-direction.md`
**Plans:** 67 completed (`completed/`) · 5 open (below) · proposals in `../proposals/` ·
...
| ~~[0077](./completed/0077-doctor-promote-it.md) — `doctor`: promote it~~                          | **DONE, and reversed during its own investigation.** Draft 1 said retire, on the ground that `doctor` cannot load the shape the docs lead with. Measured: it cannot load a vitest-importing file (true), but `getting-started.md:10` says **the CLI is the default path** and `init` scaffolds a rule file with zero vitest imports — so `doctor` works on the documented default. What earns it a slot is a **dead selector glob**: `check` never calls `diagnose()`, so it exits **0 with no output** on a rule that can never match while `doctor` exits 1 and names the site. A second false claim — that only `doctor` catches load failures — was written into the plan and caught by review: `check` catches those too, exits 1, and prints a remedy. Promoted to `--help` with its scope stated; `--format` validated. **6 of 6 sabotages**, two of which first scored MISSED against a revert string that matched nothing. Still unblocks [0074](./0074-r3b-the-selector-glob-flip.md). | ~2 hours    | a decision                                |
| [0074](./0074-r3b-the-selector-glob-flip.md) — R3b: the selector glob flip                        | **GATE OPEN, blocked on two bugs.** Gate run 4 (2026-07-31) ran the pre-flight on both sides. This repository: 36 rules collected as values, **zero findings** — refuted as the gate codebase for the fourth time, and the population is 36, not the 43 recorded here and in the plan. `honojs/hono` with the published 0.32.0 and the unedited `init --preset layered` scaffold: **10 findings**, and **6** with the tsconfig repointed at one that loads. Against the decision rule — does any message assert a cause wrong for that input? — **yes, twice**: [0031](../bugs/0031-diagnose-blames-the-glob-when-the-project-loaded-nothing.md) and [0032](../bugs/0032-an-absent-path-defers-to-a-cause-list-it-refutes.md). **Both shipped in v0.33.0**, so the messages R3b converts into build failures have been corrected and verified against the codebase that found them. Breaking.                                                                                                    | ~1 day      | bugs 0031 + 0032, then version sequencing |
