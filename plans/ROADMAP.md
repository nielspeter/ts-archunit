...
| [0030](../bugs/0030-user-defined-predicates-and-conditions-cannot-declare-globs.md) — user-defined predicates and conditions cannot declare globs     | **Open.** `definePredicate` / `defineCondition` return only the fields they are handed and have no `globs` parameter, so a custom path-matching predicate's glob never reaches `doctor`. Split severity, and the split is the point: **selector** position is acted on today, so the predicate half is a present-tense detection gap; **condition** position is skipped by decision, so the condition half is latent until [0074](./0074-r3b-the-selector-glob-flip.md). Surfaced by [0073](./completed/0073-conditions-declare-their-globs.md), which fixed the built-in side and left the public factories asymmetric. Fix both together or the higher-severity half stays open behind a closed-looking bug. |
