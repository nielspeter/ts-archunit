...
| ---------------------------------------------- | -------------------------------------- | -------- | --------------------------------------------------------------------------------------------------- |
| 0069 — No rule may certify nothing             | **R-any/R1/R2a/R3a SHIPPED** (v0.20.0) | ~2 days  | R2b unblocked; R3b designed, gated on an adopting codebase running `doctor` — possible from v0.20.0 |
...
| [0011](../bugs/0011-dogfood-rules-select-nothing.md) — ~~14 dogfood rules select nothing~~ **FIXED v0.20.0** (17, not 14) | **Open**, and its fix is now known: the 0067-C empty-selector flip detects all 13 automatically. No bespoke guard needed — see below.                                                                                            |
...
| [0016](../bugs/0016-narrowing-a-named-selection-mutates-it.md) — narrowing a named selection mutates it | **Open.** `should()` forks, `that()` does not, so a held selection loses subjects and later rules pass silently. Found by plan 0069's measurement — the test named `named selection reuse works` had been demonstrating that it does not. Wider than filed: `and()`, `excluding()`, `rule()` mutate too, and GraphQL's `SchemaRuleBuilder` forks in neither method. **Should land before R3b**, whose migration note says "these findings are true". |
| [0018](../bugs/0018-data-layer-preset-silently-enforces-nothing-for-a-file-glob.md) — ~~a preset enforced nothing for a file glob~~ | **FIXED v0.20.0.** `repositories`/`shared`/layer globs went to `resideInFolder`, which reads the parent directory, so a file glob matched nothing and the preset checked nothing. |
