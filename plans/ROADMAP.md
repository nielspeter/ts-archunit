...
| [0025](../bugs/0025-a-non-archruleerror-from-one-rule-file-drops-every-other-finding.md) — one bad rule file drops every finding in the run     | **Open.** `runCheck` catches `ArchRuleError` only, so any other error escapes the per-file loop and no report is written. Reachable today via `CorrespondenceBuilder`'s arity `RangeError` — which v0.23.0 deliberately kept, since wrong arity is a different fault from a missing assertion. Fix the fault class at the CLI boundary, as `runDoctor` already does for unloadable files. Found by two independent reviews of v0.23.0. |
| [0026](../bugs/0026-a-location-less-finding-does-not-say-which-rule-file-it-came-from.md) — a location-less finding is not locatable            | **Open.** A configuration finding has `file: ''`, so two identical vacuous rules in two rule files render as two identical paragraphs. The test form is fine (vitest supplies the frame); the CLI form is not, and v0.23.0 made this the first failure a whole population meets. The attribution exists in `runCheck`'s per-file loop and is discarded.                                                                                |
| [0027](../bugs/0027-an-unmatched-baseline-entry-cannot-be-diagnosed.md) — an unmatched baseline entry cannot be diagnosed                       | **Open.** `unmatchedBaselineFinding` is gated on `matched === 0`, so a partial miss is silent and the entry surfaces as a new violation. Deliberate — "some entries did not match" is also what success looks like — but it leaves the common case unexplained. Needs a differently-derived signal (the stored `rule` string), not a version label: v0.23.0 tried the label and withdrew it.                                           |
