...
**Plans:** 61 completed (`completed/`) · 7 open (below) · proposals in `../proposals/` ·
...
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| [0069](./0069-no-rule-may-certify-nothing.md) — No rule may certify nothing             | **R-any/R1/R2a/R3a SHIPPED** (v0.20.0). Remaining: **R2b** (docs sweep, unblocked, off the critical path) and **R3b** (the glob guard + `emptyIsPass`, still gated on an adopting codebase running `doctor` as a pre-flight). The "new fault" recorded 2026-07-30 was **refuted** — a tally cannot tell a typo'd denylist from a respected ban — and moved to [plan 0072](./0072-a-denylist-glob-that-cannot-match.md). R3a's own unbuilt half is [bug 0029](../bugs/0029-a-throwing-warn-truncates-the-rest-of-the-rule-file.md).                                                                                                      | ~2 days  | R2b unblocked; R3b = the glob guard + `emptyIsPass` + the new fault, still gated on an adopting codebase |
...
| [0073](./0073-conditions-declare-their-globs.md) — Conditions declare their globs       | **READY**, and small: `Condition.globs` and `RuleBuilder.globs()`'s gathering already exist and **zero** conditions populate them. Seven conditions, one line each. Behaviour-neutral — measured, 2580 tests pass unchanged — which is also the risk, so the guards assert the declaration itself. Makes condition globs visible to `explain`, `doctor` and 0069's glob model.                                                                                                                                                                                                                                                          | ~0.5 day | nothing                                                                                                  |
...
| [0029](../bugs/fixed/0029-a-throwing-warn-truncates-the-rest-of-the-rule-file.md) — a throwing `.warn()` truncates the rest of the rule file          | **FIXED.** The CLI reports the truncation — the half [plan 0069](./0069-no-rule-may-certify-nothing.md) R3a specified and shipped without — and each finding is reported once. Scoped to a thrown terminal: for a syntax error nothing ran, so the notice would imply otherwise. Guarded against real rule files on disk, with the `export default [...]` shape as the control.                                                                                                                                                                                                                                        |
