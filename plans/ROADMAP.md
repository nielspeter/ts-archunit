...
| [0077](./0077-retire-doctor-keep-the-diagnosis.md) — Retire `doctor`, keep the diagnosis          | **PROPOSED — a decision, not a feature.** 0069 required `doctor`'s keep-or-retire call **before R3** and named the drift mechanism: shipping it experimental/hidden defers the decision. It is hidden from `HELP_TEXT` and documented in seven pages. The deciding fact is that it must `import()` the rule file, so it **cannot load a file that imports vitest** — the shape CLAUDE.md calls primary — and its own error tells the reader to use the test suite instead. Every finding it reports is `diagnose()`'s, which is already public. Recommends retire-in-two-releases; names promote-instead as the coherent alternative. **Unblocks [0074](./0074-r3b-the-selector-glob-flip.md)**, whose gate is blocked only because this repo cannot run `doctor` on its own 43 rules. | ~half a day                | a decision                              |
...
65 plans in `completed/`, numbered 0000–0076 (counted, not incremented). Accepted proposals that shipped without
