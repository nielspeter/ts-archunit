...
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ----------------------------------------- |
| [0076](./completed/0076-resolve-module-edges-once-per-file.md) — Build module edges once per file | **DONE (unreleased).** `edgesOf()` rebuilt a file's edges per call: five whole-project `notImportFrom` rules made **10,545 `getSymbol` calls in 46ms** where one pass would do. Caches the **built edges**, not resolution — measured, of 48ms across five passes the walk is 1ms, resolution 15ms and construction ~32ms, so this file's own "cache resolution, not the walk" docstring was aimed at the middle third. The dangerous part is invalidation: a `SourceFile`'s identity **survives an edit**, so 0075's key argument does not transfer — `onModified` (ts-morph, ADR-002 intact) invalidates per file. `edgeStream` reads the cache warm and never fills it cold, preserving `dependOn`'s early exit. 6/6 sabotages caught. | ~half a day                | done                                      |
| [0075](./completed/0075-collect-elements-once-per-project.md) — Collect elements once per project | **DONE (unreleased).** `filterElements()` re-collects from scratch on every rule execution. Measured on this repo (518 files): **5 × `calls()` = 2,590 descendant queries / 684ms where 518 / 131ms would do** — 5.2x waste that scales with rule count. A prototyped memo gets it to 518 / 144ms (**4.75x**) with all 31,560 subjects identical elementwise. Also covers `agentGuardrails` (8 banned APIs: 142 → 83ms), which is the commoner case because it is a preset we ship. No behaviour change. Four measured constraints, the load-bearing one being **memoize `getElements()`, never `filterElements()`** — a `filterElements()` memo makes `within(sel).functions()` share a key with `functions()` and pass vacuously.       | ~1 day                     | nothing                                   |
| ~~[0077](./completed/0077-doctor-promote-it.md) — `doctor`: promote it~~                          | **DONE, and reversed during its own investigation.** Draft 1 said retire, on the ground that `doctor` cannot load the shape the docs lead with. Measured: it cannot load a vitest-importing file (true), but `getting-started.md:10` says **the CLI is the default path** and `init` scaffolds a rule file with zero vitest imports — so `doctor` works on the documented default. It also reports **load failures, which `diagnose()` structurally cannot**, and a rule file that fails to load is zero coverage reported as success. Recommends promoting it to `--help` and replacing 'future undecided' with its scope. Still unblocks [0074](./0074-r3b-the-selector-glob-flip.md).                                                  | ~2 hours                   | a decision                                |
