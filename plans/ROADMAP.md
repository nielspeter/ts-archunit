...
**Version:** 0.32.0 · **Tests:** 2710 across 196 files · **Updated:** 2026-07-31
...
| [0074](./0074-r3b-the-selector-glob-flip.md) — R3b: the selector glob flip                        | **GATE OPEN, blocked on two bugs.** Gate run 4 (2026-07-31) ran the pre-flight on both sides. This repository: 36 rules collected as values, **zero findings** — refuted as the gate codebase for the fourth time, and the population is 36, not the 43 recorded here and in the plan. `honojs/hono` with the published 0.32.0 and the unedited `init --preset layered` scaffold: **10 findings**, and **6** with the tsconfig repointed at one that loads. Against the decision rule — does any message assert a cause wrong for that input? — **yes, twice**: [0031](../bugs/0031-diagnose-blames-the-glob-when-the-project-loaded-nothing.md) and [0032](../bugs/0032-an-absent-path-defers-to-a-cause-list-it-refutes.md). Both must be fixed first, because R3b's content is turning exactly these strings into build failures. Breaking.                                                                                                                                                   | ~1 day      | bugs 0031 + 0032, then version sequencing |
...
| Bug                                                                                                                                                         | State                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [0031](../bugs/0031-diagnose-blames-the-glob-when-the-project-loaded-nothing.md) — `diagnose()` blames the glob when the project loaded nothing             | **OPEN.** Found by 0074's gate run 4 against `honojs/hono` with the published 0.32.0 and the unedited `init --preset layered` scaffold. Hono's root tsconfig is `"files": []` plus references, so it loads nothing — `tsc --listFilesOnly` agrees. `check` names that cause correctly; `doctor`, same run, blames each of 6 globs instead. `slice-rule-builder.ts:345` already states the rule that `diagnose()` does not apply: _"blaming the glob would send the caller to the wrong file entirely."_ Blocks [0074](./0074-r3b-the-selector-glob-flip.md), which turns this text into a build failure.                                                                  |
| [0032](../bugs/0032-an-absent-path-defers-to-a-cause-list-it-refutes.md) — an `absent` path defers to a cause list it refutes                               | **OPEN.** Same gate run, project loading normally: 6 findings carry `onDisk: 'absent'`, whose advice is `''`, so they fall through to `no-match`'s three causes — two of them refuted by what `absent` means (there is no directory, so "append `/**`" and "holds no source files" are both false). `ON_DISK_ADVICE`'s own docstring states that principle and applies it to the other two known-fact cases. `not-determined`'s empty string is correct and must stay. Also blocks [0074](./0074-r3b-the-selector-glob-flip.md).                                                                                                                                          |
