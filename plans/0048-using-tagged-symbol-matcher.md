...
- **State:** PROPOSED — **go/no-go 2026-07-14: DEFER UNTIL DEMAND.** Full-panel proposal review: the engineering is strong (Phase 0 spike, `ExpressionMatcher` reuse, alias unwrapping all correct) but it does not earn its place now. It overlaps `@typescript-eslint/no-deprecated` (stable, zero-config, editor-time — the plan itself concedes this wins the 90% case); the differentiated slice is thin (folder-scoping = one eslint `overrides` block); and the "future plans depend on the symbol layer" justification is speculative (no such plans exist). Heaviest remaining work for the lowest value. **Defer trigger:** revisit when (a) a user asks for architecture-scoped deprecation eslint can't express, or (b) a SECOND symbol-resolving matcher is proposed with independent value (then extract `symbol-resolution.ts` for two real consumers). If built later, fix the cache design to a module-level `WeakMap<Node, Symbol>` (per `reverse-dependency.ts`), not per-instance closures. Design preserved at near-zero carrying cost.
- **Cross-reference, added 2026-08-13, decision unchanged:** trigger (a) hasn't fired (no one has asked for architecture-scoped `@deprecated` enforcement). Trigger (b) arguably has a live candidate: [proposal 028](../proposals/028-flow-boundary-mechanism-rules-cannot-anchor.md) needs its own symbol-resolution plumbing (`onResultOf`'s resolver) for a different capability, and its own text names the exact coordination risk this plan's trigger anticipated — "two independently-built symbol-resolution paths landing at different layers for the same underlying capability." Neither this plan nor 028 is built. There is also a third dormant thread: the original spec's `symbolOf()`/`resolvesTo()` (§6.3.1, deferred as [plan 0014](./completed/0014-call-entry-point.md)'s Phase 2, never scheduled). If 028 is ever picked up, that is the moment to design the shared `symbol-resolution.ts` extraction and reconsider this plan and `symbolOf()`/`resolvesTo()` together, rather than three separate re-litigations. This does not change the deferred verdict above — it records where the trigger condition currently stands.
