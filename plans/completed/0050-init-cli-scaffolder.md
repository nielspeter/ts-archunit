...
- **State:** Done (v0.14.0, branch `feat/0050-init-cli-scaffolder`) — `runInit` + dispatch/parseArgs/HELP_TEXT wiring, source-root detection, package.json merge (indent/EOL preserving), both returning-form presets, all message/CI fixes; 21 tests + drift guard; docs (cli/getting-started/README) + CHANGELOG. Verified end-to-end (scaffold → generated files correct). Shape presets excluded from v1 per round-2 review.
