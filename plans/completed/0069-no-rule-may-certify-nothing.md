...
**Status:** **DONE — R2b shipped 2026-07-30; R3b split out to [plan 0074](./0074-r3b-the-selector-glob-flip.md).**
...
**`emptyIsPass`, specified** — it was previously named only as "`.some()` → `.every()`", which lived on the `spike/0067c-empty-by-default` branch and not in this plan, making it unimplementable from here. **Shipped in v0.34.0** as part of R3b ([plan 0074](./0074-r3b-the-selector-glob-flip.md)); the spike branch has been deleted, and its prototype — a public `emptyIsPass?: boolean` on `Condition` — was deliberately superseded by a module-private symbol, for the reason [the appendix](./0069-appendix-vacuous-tests.md) gives: a public flag is a one-line silent opt-out on any user condition. The contract: a condition today reports a violation when **some** subject fails; over an empty subject set that is vacuously false, so the rule passes. `emptyIsPass` inverts the default — a condition is satisfied only when **every** subject satisfies it _and_ at least one subject exists — so an empty subject set fails instead. It is opt-out per condition for the genuine cases (`.notExist()` rules, where zero subjects is the passing state), it **never covers a path fault** (an unsatisfiable glob is caught earlier, by the tree, with a different message), and it lands in R3b. Its doc surface is `docs/core-concepts.md`'s condition semantics section.
...
**R3b does not ship until** the adopting codebase has run R2a's pre-flight and its findings have been classified by remedy. **Fallback: R3b slips, and R3a ships without it.** The fallback for a missing gate is a slip, not a weaker gate. Splitting a and b is what stops an indefinite external slip holding open a live false-green hole on findings that ship today.

> **Two corrections, 2026-07-30, both measured.** The sentence this paragraph used to carry — "all 35 of this repo's path-glob sites were written by someone who knew the guard was coming and cannot falsify it" — is **false as stated**: `tests/archunit/arch-rules.test.ts` was created 2026-03-26 and this plan was first committed 2026-07-25, four months later. What is true is narrower: `ea82483 fix(arch): rescope our own rules so they enforce something (bug 0011)` rewrote its globs on 2026-07-26, so the population is **mixed**, not uniformly curated.
>
> **The exclusion stands anyway, for a different and stronger reason.** Dogfooding was attempted: `spikes/0069-gate-walk.mjs` run across all 34 fixture projects plus the root.
>
> ```
...
> ```
>
> The fixture corpus is a **vacuous population**: 33 projects contribute nothing to classify, because fixtures are minimal by construction — small, flat, everything inside their own tsconfig. Of the 36 absent directories that do exist, 33 are `.claude/`, `.claude/skills/…` and `schema/`, where "contains no TypeScript" is trivially correct and tests nothing. Only **3** reach the interesting category, and they come from the root project. Gate run 2 needed a 215-file monorepo with a Rust crate in it to surface 3 interesting cases; this repository surfaces 3 from the one project the gate excludes.
>
