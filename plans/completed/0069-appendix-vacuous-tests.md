...
## B — asserts the current default, and R3b invalidates it by design (6)
...
| `RuleBuilder: expectNonEmpty() does not leak onto the held selection`      | **Added 2026-07-28** by the bug 0016 guards, not present at derivation. Its second half is `expect(() => nothing.should().beExported().check()).not.toThrow()` on an empty selection — the negative control proving the opt-in is per rule. Legitimate, and R3b inverts it by design: invert, do not delete. Filed here so R3b does not meet it as a surprise red in a file whose name suggests it has nothing to do with vacuity. |
...
| `passes when only good repo and baseClass not specified`                                                   | **Was filed under A**, on a rationale measurement contradicts: the preset does generate the rule and it does apply — it selects nothing, because `repositories` takes a file glob and `resideInFolder` reads the parent directory. A live shipped defect, now [bug 0018](../../bugs/fixed/0018-data-layer-preset-silently-enforces-nothing-for-a-file-glob.md). Had R3b been built to leave category A untouched, the guard would have been designed to preserve it. |
...
**All 8 in C are fixed** (2026-07-26), and the population measured **35 → 28**. Re-measured at v0.21.0 with the same recipe: **29 tests across 16 files** — 28 plus one entry this work added (see B).

Every number in this block used to be the **first cut** that the header above retracts: "the six in C", "26 in A", "1 in B", "35 → 30". The corrected split is 22 A / 6 B / 8 C, and 22 + 6 = 28 reconciles with the measurement — 7 of the 8 C entries left the population and one was rewritten into a legitimately-empty test that stays. The Status section was simply never updated after the correction, which is the same defect the document exists to describe, in the document itself. It is now stated as a measurement with the command that produces it, not as a total.

**Bug 0016 changed no classification.** It was R3b's stated precondition, and the plan's status line asked for a re-read of all 35 entries on the theory that some were empty only because of a leaked narrowing. Measured instead of read: 28 failures at v0.20.0, 29 at v0.21.0, zero entries left the population and exactly one entered — the one this work added. Not one A or B row was empty because of the leak. The re-read finds nothing; the re-run is the answer.

Two of the eight turned out to be a **library** defect rather than a test defect — see [bug 0016](../../bugs/fixed/0016-narrowing-a-named-selection-mutates-it.md). `named selection reuse works` was demonstrating that it does not: narrowing a named selection mutates it, so the second rule lost the subject it was written for. `inherits all FunctionRuleBuilder predicates and conditions` had the same shape twice over — its `contain()` assertion ran on the set that its own `areAsync()` filter had already emptied.
