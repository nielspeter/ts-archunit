...
- **State:** Done (v0.15.0, branch `feat/0055-tsconfig-rule`) — `tsconfig(p).requires()` builder, strict-family resolution (9 flags incl. `strictBuiltinIteratorReturn`, guarded by a size test), enum-name messages, deep array/object compare, extends + workspace-primary + baseline verified; 22 tests; docs (`config-rules.md` + api-reference + README) + CHANGELOG. **Interface (adjusted after review):** `getCompilerOptions()` is added to `ArchProject` as an **optional** method (a required one would break ~58 inline test literals); `project()`/`workspace()` implement it, and the builder prefers it with a `_project` fallback for bare literals. (`tsconfig-builder` is the first `_project` reader — a deliberate, type-safe seam, not existing precedent.) Phase 4 (EESS walkthrough) skipped — that asset isn't in this repo.
