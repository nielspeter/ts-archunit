...
2. **Do NOT gate `docs.yml`.** Reversed from draft 3, on measurement: `npm test` is **~103s on a 4-vCPU runner** (my 13.9s was a 14-core local number — `user 143.94` vs `real 13.72`), so the docs job goes 29s → ~132s, a **4.5x** increase, not the "+15s" the draft claimed. And the gate does not do what draft 3 said: `docs.yml` has `paths: ['docs/**']`, so it never fires for the src-side case at all. `ci.yml` already runs `npm test` on every push to `main` **and** every PR — the rot fails there.
   ...
   **This is not an exception to [ADR-008](../../adr/008-agent-first-failure-surfaces.md) rule 1** — it is the distinction the rule draws. Rule 1 forbids a **false green** (an artifact ships and _no check ever fails_). Here a check does fail, ~1m48s later, so what remains is a **delayed red**: a ~2-minute exposure window on the direct-docs-push-to-`main` path only. That is a cost to weigh, and 4.5x on every deploy — plus the `concurrency` group `docs.yml` lacks, whose collision window the change would widen 4.5x — is not worth it. **Keeps this plan `tests/`-only.**
...
- **Recipe-page extraction** — publish the pattern as a user-facing docs recipe ("fail CI when your docs teach deprecated API") **after the guard has run in anger**. The drift class is structural (our own docs were correct at birth; one `src`-side deprecation commit rotted 27 usages for 109 days), so every evolving library with API-bearing docs has it. Zero API surface: the recipe is copyable test code, and its examples must use synthetic names (as the Phase 2 tests do) so the recipe page cannot trip the guard itself. If demand appears, the extractable _primitive_ is `getDeprecatedTag(node)` / a `deprecatedSymbols(p)` helper (proposal 013's stated extraction trigger) — the text scan itself stays a recipe, never API (the 012 precedent).

  _Status 2026-08-01: still out of scope, and the trigger has not been met — the guard has been on `main` since 2026-07-17 and has not fired once. This entry was written on the original `0063-docs-deprecation-scan` branch and stranded there when the work landed via PR #3 from `feat/0063-docs-deprecation-scan`; it is recorded here so the branch can be deleted without losing the decision._
