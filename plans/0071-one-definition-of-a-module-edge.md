...
**Status:** **0.27.0 SHIPPED, 0.28.0 BUILT AND UNRELEASED** (2026-07-30) — the three instruments are released; §Test inventory items 1-3 are done, with 29 tests and a 15-revert sabotage matrix. 0.28.0's implementation is complete on `feat/0071-r2-widen-module-edges` — 21 reverts derived from the diff, all caught — and is **held for a release decision**, since it changes enforcement for every consumer. Drafts 1–3 each had claims measured wrong; that history is in this branch's commit messages and in the two bug files, not here. **Draft 4 exists because two independent reviewers, working from opposite ends, found the same thing: item 7 — the release's headline guard — was green with bug 0022 fully reinstated.**
