...
| `passes when only good repo and baseClass not specified`                                                   | **Was filed under A**, on a rationale measurement contradicts: the preset does generate the rule and it does apply — it selects nothing, because `repositories` takes a file glob and `resideInFolder` reads the parent directory. A live shipped defect, now [bug 0017](../bugs/0017-data-layer-preset-silently-enforces-nothing-for-a-file-glob.md). Had R3b been built to leave category A untouched, the guard would have been designed to preserve it. |
...
1. **The 8 in C are fixed.** Two turned out to be library defects rather than test defects — bugs 0016 and 0017.
