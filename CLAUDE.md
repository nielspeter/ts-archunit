...
| [010](./adr/010-the-extension-surface-is-a-contract.md)   | Extension Contract | _Proposed._ External dialects (eess) subclass the core builders, so the protected extension surface — `copy()`, `collectViolations()`, `assertsSomething()`/`assertionAdvice()`, `_reason`/`_metadata` — is a named contract: breaking it is a breaking change, guarded by a foreign-dialect fixture built from the published `.d.ts`. Twin of eess ADR-009; ratified together. |
