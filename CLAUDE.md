...
| [007](./adr/007-isolate-ast-engine-boundary.md)           | Engine Boundary    | _Proposed._ Confine ts-morph behind `src/core/engine/`; talk to the engine in batches, not per-node.                                                                                                                                                                                                                                                                                                                                                                                      |
| [008](./adr/008-agent-first-failure-surfaces.md)          | Agent-First        | **A check that cannot fail is counted as coverage, and that is a lie.** Findings fail when the remedy is not optional, carry their own _verified_ remedy, no snapshot pins — and **a derivation is unguarded until a differently-derived value disagrees with it**, enumerated from the diff. Rule 6 bounds the regress: **chase it as deep as the blast radius, then stop.**                                                                                                             |
| [009](./adr/009-a-pass-is-constructed-from-evidence.md)   | Evidence           | _Proposed._ **A pass is constructed from evidence of examination** — a non-empty examined-unit set counted at the family's own seam, or a declared-empty token (`.notExist()` / `.expectEmpty()`), which an empty project outranks. Vacuity is unrepresentable: enforced by the terminal seam's type plus a vacuity matrix enumerated from the `package.json` exports map.                                                                                                                |
| [010](./adr/010-the-extension-surface-is-a-contract.md)   | Extension Contract | _Proposed._ External dialects (eess) subclass the core builders, so the extension surface is a named contract — `copy()`, `collectViolations()`, `_reason`/`_metadata` (protected) plus `assertsSomething()`/`assertionAdvice()`, `declaresEmpty()`, `emptyDeclarationAdvice()` (public, because `diagnose()` reads them structurally). Breaking it is a breaking change, guarded by a foreign-dialect fixture built from the published `.d.ts`. Twin of eess ADR-009; ratified together. |
...
- **ADR-008 (Agent-First):** the danger is a green that means nothing — a rule whose glob matches no files passes, a selector that narrows to nothing passes (∀ over ∅ is true), and the suite counts both as coverage. Our primary consumer is an AI agent: it does not read warnings, and it invents a remedy when none is stated. So a finding whose remedy is **not optional** must fail (a finding the reader must judge should warn — that distinction is the rule, not "never warn"); every failure carries a remedy that has been **verified to remediate**, not merely to read well; and before shipping any guard, ask the one question that matters: **"what would this test do if the thing it guards were completely broken?"** If the answer is "pass", the derivations are not independent — and ask it against a revert list derived from the **diff**, because asked against a list from memory it answers "pass" honestly and tells you nothing. How far to carry that is **rule 6**: guard the guard for published API and irreversible effects; for an internal check over a corpus we control, prove each detector fires once and stop.
