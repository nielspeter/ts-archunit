...
### Changed (⚠️ two new failures, both replacing a silent pass)

- **A slice rule that resolves exactly ONE non-empty slice now fails.** Every slice condition compares slices to each other — `beFreeOfCycles` does not look at dependencies _inside_ a slice — so a single slice makes the rule unfalsifiable. This is not hypothetical: a mis-parsed glob collapsed a real cycle into one slice twice during this fix, and every other guard stayed quiet. Broaden the glob (`'src/features/*'` rather than `'src/features'`) or drop the rule.
- **A slice that matched no files now fails even when its siblings matched.** The conditions silently skip slice names they cannot find, so one typo in a multi-layer `assignedFrom()` left that layer unchecked while the build stayed green. The finding names the empty slices.
- **Configuration findings can no longer be downgraded with `.asSeverity('warn')`.** They report that the rule enforces nothing, and a warning is invisible to an agent consumer (ADR-008). Exclusions and baselining already refused them; this closes the last silencer. `.asSeverity('warn')` still applies to real violations of the same rule.
