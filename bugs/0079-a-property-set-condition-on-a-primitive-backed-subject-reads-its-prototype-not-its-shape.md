...
| `havePropertyMatching(/^c/)` on the type above          | **passes silently, 0 violations** — `charAt` (a `String.prototype` method, not anything the author wrote) matches `/^c/`, so the condition reports "yes, a property matches" and never inspects whether the _intended_ property exists                                                                                                                          |
