...
**Version:** 0.56.0 · **Open:** 4 · **Fixed:** 62 (`fixed/`) · **Updated:** 2026-08-05
...
0056 and 0062 came out of the five-persona review of v0.47.0–v0.49.0; 0064 and 0065 out of the
five-persona review of the bug-0059 branch, which found them **pre-existing** rather than introduced —
neither is a regression in the release that surfaced them.
...
| [0062](./0062-the-release-pipelines-gates-drift-and-its-diagnostics-misname-the-cause.md) — the release pipeline's gates drift                                            | Medium   | ~~`shellcheck` is in `ci.yml` and not `publish.yml`~~ — **that half is now fixed**, `shellcheck` runs in `publish.yml`, though as a hand-maintained second copy rather than the reusable workflow the bug prescribed, so the drift is reset and not prevented. Still open: a 429 reds the docs step naming four wrong causes; that step's timeout is budgeted at 600s against a 720s worst case; the docs site deploys independently of publish success with no concurrency group. **Verified sound:** a tag cannot publish a failing commit, and no fixture leaks into the tarball.                                                                                                                                                                                                                                                                                                                                                                                          |
| [0064](./0064-a-dependency-identity-collides-across-two-spellings-of-one-module.md) — one baseline entry silently accepts a second import of the same module              | **High** | Identity keys on the **resolved** path while every discriminator derives from the **specifier**, so two spellings reaching one file — an alias, or a workspace package name beside a relative path, needing no `paths` at all — give **2 findings / 1 hash**, and one accepted entry pre-accepts a different edge. Bug 0028's shape. Collides for `dynamic`, named, **default** and **type-expression**; only differing names save it. `notImportFrom`/`onlyImportFrom` affected, **`dependOn` refuted** — it emits one violation per file and cannot collide. **Pre-existing, not a v0.56.0 regression**, reproducing under the pre-ordinal discriminator; **module-family only**, since the slice family keys on the raw specifier and is correct — so "make the families consistent" could propagate it. **The remedy first filed was refuted by measurement** (it moved 129 of 975 identities to close 3); the report now carries two measured constructions that move 3. |
| [0065](./0065-reverse-dependency-findings-carry-no-identity.md) — `noDeadModules`/`beImported`/`onlyBeImportedVia`: two files sharing a filename share one baseline entry | **High** | Reverse conditions set no `identity`, so `hashViolation` falls back to `element::message` with `element` a **basename**. Measured **2 findings / 1 hash** for `onlyBeImportedVia`, for `beImported`/`noDeadModules`, and for `haveNoUnusedExports`. **Reaches further than 0064**: that one needs a file to import one module twice by two spellings; this needs only two files anywhere in the selection to share a filename — and a dead-module sweep is full of `index.ts`, `types.ts`, `utils.ts`. `beImported`'s own docstring recommends `.excluding('index.ts', 'main.ts')`, pointing at the colliding population. Bug 0063's defect in the family that never got 0063's fix, and **not limited to that family** — 9 basename-element sites across 4 files, so derive the census rather than listing it. **A zero-migration construction exists**; the report's original "the fix is not free" claim was false.                                                        |
