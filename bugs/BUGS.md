...
neither is a regression in the release that surfaced them. Every claim below was reproduced by
measurement before filing.
...
| [0064](./0064-a-dependency-identity-collides-across-two-spellings-of-one-module.md) — a dependency identity collides across two spellings of one module | **High** | Identity keys on the **resolved** path while every discriminator derives from the **specifier**, so a `paths` alias beside a relative import gives **2 findings / 1 hash** and one accepted entry pre-accepts a different edge. Bug 0028's shape, on the common monorepo layout. **Pre-existing, not a v0.56.0 regression** — it reproduces with named imports under the pre-ordinal discriminator — and **module-family only**: the slice family keys on the raw specifier and is accidentally correct, so "make the families consistent" could propagate it.                       |
| [0065](./0065-reverse-dependency-findings-carry-no-identity.md) — reverse-dependency findings carry no identity                                         | **High** | `beImported` and `onlyBeImportedVia` set no `identity`, so `hashViolation` falls back to `element::message` with `element` a **basename**: two importers named `consumer.d.ts` in sibling folders give **2 findings / 1 hash**. Bug 0063's defect in the family that never got 0063's fix. The fix moves every reverse baseline entry — no empty-discriminator trick exists here — so it needs a stated migration, and is worth batching with 0064.                                                                                                                                  |
