...
**Version:** 0.49.2 · **Open:** 9 · **Fixed:** 54 (`fixed/`) · **Updated:** 2026-08-04
...
All nine came out of the five-persona review of v0.47.0–v0.49.0 (except 0054, which those releases found).
Every claim below was reproduced by measurement before filing.

| Bug                                                                                                                                             | Severity     | What                                                                                                                                                                                                                                                                                                                                                         |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [0055](./0055-a-cycle-finding-names-edges-that-do-not-exist.md) — a cycle finding names edges that do not exist                                 | **High**     | An SCC is printed as a path. Measured on a ring `a→b→c→d→a`: reports `a -> d -> c -> b -> a`, every arrow reversed, at `unknown:0`. On our own source 2 of 4 arrows are fabricated and the closing edge is not named. Latent until 0084 made the rule `.check()` and 0085 made multi-member cycles normal.                                                   |
...
| [0058](./0058-workspace-applies-one-packages-compiler-flag-to-all.md) — `workspace()` applies one package's compiler flag to all                | **High**     | `verbatimModuleSyntax` is read from the alphabetically-first tsconfig and applied to every package. Measured both directions: a real cycle vanishes, and a phantom cycle reds CI with a remedy that cannot remediate it. Bug 0035's shape in the file that already fixed it for globs.                                                                       |
| [0060](./0060-a-pattern-change-silently-invalidates-every-baselined-finding.md) — a pattern change silently invalidated every baselined finding | **High**     | The rule description interpolates `String(pattern)` and `hashViolation` hashes the rule, so v0.47.0's `STUB_PATTERNS` rewrite moved every stub hash. Measured: 0 of 4 entries matched. The diagnostic then blames the repository root with confidence.                                                                                                       |
...
| [0057](./0057-an-empty-options-object-reverts-a-documented-default.md) — an empty options object reverts a documented default                   | Medium       | `beFreeOfCycles()` → no cycle; `beFreeOfCycles({})` → cycle. The default is on the whole object, the read is per field. Worse the moment `ImportOptions` gains a second field.                                                                                                                                                                               |
...
| [0054](./0054-within-makes-helpers-depend-on-builders.md) — `within()` makes `helpers/` depend on `builders/`                                   | Low / Medium | The one cycle in our source, waived by identity. Its own "fail-closed" claim is disproven by 0056 and needs correcting when that lands.                                                                                                                                                                                                                      |
