...
**Version:** 0.52.0 · **Open:** 6 · **Fixed:** 58 (`fixed/`) · **Updated:** 2026-08-04
...
Five came out of the five-persona review; 0063 came out of reviewing v0.52.0 of v0.47.0–v0.49.0 (except 0054, which those releases found).
...
| [0063](./0063-a-dependency-identity-collides-across-files-sharing-a-basename.md) — a dependency identity collides across files sharing a basename | **High**    | Two distinct findings share one baseline entry, so accepting either accepts both — the exact requirement `ArchViolation.identity`'s docstring states. Measured: two sibling feature folders each with an `index.ts` re-exporting the same name give `findings=2 distinctHashes=1`. Bug 0028's shape one step over: it fixed two findings in ONE file, this is two in TWO files. Found by reviewing v0.52.0, because the slice conditions copied this scheme and inherited it — and fixed on the slice side in that release by using the full path. |
