...
| `.assertsSomething()` | `.assertsSomething(): boolean`                                     | Whether this rule asserts anything about what it selects (0.22.0). `false` means the rule can never fail; the assertion gate warns on it now and fails on it from 0.23.0. `diagnose()`/`doctor` read it. External `TerminalBuilder` subclasses are **exempt by default** (`true`) — override it if your builder has an assertion-less state.                                             |
| `.assertionAdvice()`  | `.assertionAdvice(): string`                                       | The remedy for this builder's assertion-less state, per state (0.22.0). One string, one place: the runtime warning, `doctor`'s advice and the eventual failure message all read it verbatim.                                                                                                                                                                                             |
