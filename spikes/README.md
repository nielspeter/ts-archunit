...
| `022-duplication-report-shape.mjs` | Proposal 022's four counts — pairs vs groups, the largest single-file contribution, and how many findings carry a `suggestion` |
| `022-signals-scored.mjs`           | Proposal 022's signal table — every candidate signal scored against 153 hand labels in `022-labels.json`                       |
...
node spikes/022-signals-scored.mjs          # self-contained; scores the committed labels
npm run build && node spikes/022-duplication-report-shape.mjs ../cmless
```

`022-labels.json` is the only **data** file here, and it is the reason the scoring
script is worth keeping: proposal 022's central negative claims — that the shipped
similarity score carries no information about whether two functions do the same
thing, and that cohesion is +0.0 — are unreproducible without the labels. Someone
who disagrees with a label can flip it in that file and re-run.
